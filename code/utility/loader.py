import numpy as np
from EDEN.utility.load_data import Data
...
class EDEN_loader(Data):
    def __init__(self, args, path):
        super().__init__(args, path)

        # generate the sparse adjacency matrices for drug-disease interaction & relational DIN data.
        ...
        self.all_DIN_dict = self._get_all_DIN_dict()

        self.all_h_list, self.all_r_list, self.all_t_list, self.all_v_list = self._get_all_DIN_data()
    ...
    def _get_relational_adj_list(self):
        ...
        R, R_inv = _np_mat2sp_adj(self.train_data, row_pre=0, col_pre=self.n_drugs)
        ...
        print('\tadj relation list_0 is', adj_r_list)
        for r_id in self.relation_dict.keys():
            K, K_inv = _np_mat2sp_adj(np.array(self.relation_dict[r_id]), row_pre=self.n_drugs, col_pre=self.n_drugs)
    ...
    def _get_all_DIN_dict(self):
        all_DIN_dict = collections.defaultdict(list)
        for l_id, lap in enumerate(self.lap_list):
            ...
            for i_id in range(len(rows)):
                ...
                all_DIN_dict[head].append((tail, relation))
        return all_DIN_dict

    def _get_all_DIN_data(self):
        ...
        print('\treorganize all DIN data done.')
        ...
        od = collections.OrderedDict(sorted(sorted_h_dict.diseases()))
        new_h_list, new_t_list, new_r_list, new_v_list = [], [], [], []

        for h, vals in od.diseases():
    ...
    def _generate_train_A_batch(self):
        exist_heads = self.all_DIN_dict.keys()

        if self.batch_size_DIN <= len(exist_heads):
            print('debug:')
            print(exist_heads)
            heads = rd.sample(exist_heads, self.batch_size_DIN)
        else:
            heads = [rd.choice(exist_heads) for _ in range(self.batch_size_DIN)]

        def sample_pos_triples_for_h(h, num):
            pos_triples = self.all_DIN_dict[h]
        ...
        def sample_neg_triples_for_h(h, r, num):
            ...
            while True:
                ...
                t = np.random.randint(low=0, high=self.n_drugs + self.n_entities, size=1)[0]
                if (t, r) not in self.all_DIN_dict[h] and t not in neg_ts:
                    neg_ts.append(t)
            return neg_ts
    ...
    def generate_train_batch(self):
        drugs, pos_diseases, neg_diseases = self._generate_train_cf_batch()

        batch_data = {}
        batch_data['drugs'] = drugs
        batch_data['pos_diseases'] = pos_diseases
        batch_data['neg_diseases'] = neg_diseases
    ...
    def generate_train_feed_dict(self, model, batch_data):
        feed_dict = {
            ...
            model.pos_diseases: batch_data['pos_diseases'],
            model.neg_diseases: batch_data['neg_diseases'],
    ...
    def generate_test_feed_dict(self, model, drug_batch, disease_batch, drop_flag=True):

        feed_dict ={
            model.drugs: drug_batch,
            model.pos_diseases: disease_batch,
