...
import math
from EDEN.utility.node2vec import node2vec
...
class model(object):
    ...
    def _parse_args(self, data_config, pretrain_data, args):
        ...
        self.n_durgs = data_config['n_drugs']
        self.n_diseases = data_config['n_diseases']
        self.n_entities = data_config['n_entities']
        self.n_relations = data_config['n_relations']
        print('n_relations:'+str(self.n_relations))
        ...
        # settings for drug-disease bi network part.
        self.emb_dim = args.embed_size
        self.batch_size = args.batch_size

        # settings for DIN part.
        self.DIN_dim = args.DIN_size
    ...
    def _build_weights(self):
        ...
        if self.pretrain_data is None:
            all_weights['drug_embed'] = tf.Variable(initializer([self.n_drugs, self.emb_dim]), name='drug_embed')
            all_weights['entity_embed'] = tf.Variable(initializer([self.n_entities, self.emb_dim]), name='entity_embed')
            print('using xavier initialization')
        else:
            all_weights['drug_embed'] = tf.Variable(initial_value=self.pretrain_data['drug_embed'], trainable=True,
                                                    name='drug_embed', dtype=tf.float32)

            disease_embed = self.pretrain_data['disease_embed']
            other_embed = initializer([self.n_entities - self.n_diseases, self.emb_dim])

            all_weights['entity_embed'] = tf.Variable(initial_value=tf.concat([disease_embed, other_embed], 0),
                                                      trainable=True, name='entity_embed', dtype=tf.float32)
            print('using pretrained initialization')

        all_weights['relation_embed'] = tf.Variable(initializer([self.n_relations, self.DIN_dim]),
                                                    name='relation_embed')
        all_weights['trans_W'] = tf.Variable(initializer([self.n_relations, self.emb_dim, self.DIN_dim]))
    ...
    def _build_model_phase_I(self):
        ...
        self.u_e = tf.nn.embedding_lookup(self.ua_embeddings, self.drugs)
        self.pos_i_e = tf.nn.embedding_lookup(self.ea_embeddings, self.pos_diseases)
        self.neg_i_e = tf.nn.embedding_lookup(self.ea_embeddings, self.neg_diseases)

        self.batch_predictions = tf.matmul(self.u_e, self.pos_i_e, transpose_a=False, transpose_b=True)

    def cal_mutual_loss(self, embs_glo,embs_loc):
        target_view_embs = embs_loc
        view_embs = embs_glo
        contrast_loss = 0
        target_norm= F.normalize(target_view_embs)
        view_norm = F.normalize(view_embs)
        pos_scores = torch.sum(target_norm*view_norm,dim=1)
        ttl_scores = torch.mm(target_norm,view_norm.t())

        pos_scores = torch.exp(pos_scores/0.2)
        ttl_scores = torch.exp(ttl_scores/0.2).sum(dim=1)
        contrast_loss += -torch.mean(torch.log(pos_scores / ttl_scores))
        return contrast_loss*self.ls

    def get_glo_emb(self):
        model_glo = node2vec.Node2vec(graph=self.A_in, path_length=10,
                                  num_paths=10, dim=self.DIN_dim,
                                  workers=8, window=20, dw=True)
        emb = model_glo.vectors
        return emb

    def _build_model_phase_II(self):
        self.h_e, self.r_e, self.pos_t_e, self.neg_t_e = self._get_DIN_inference(self.h, self.r, self.pos_t, self.neg_t)
        self.A_DIN_score = self._generate_transE_score(h=self.h, t=self.pos_t, r=self.r)
        self.A_out = self._create_attentive_A_out()

    def _get_DIN_inference(self, h, r, pos_t, neg_t):
        embeddings = tf.concat([self.weights['drug_embed'], self.weights['entity_embed']], axis=0)
        ...
        # relation embeddings: batch_size * DIN_dim
        r_e = tf.nn.embedding_lookup(self.weights['relation_embed'], r)

        # relation transform weights: batch_size * DIN_dim * emb_dim
        trans_M = tf.nn.embedding_lookup(self.weights['trans_W'], r)

        # batch_size * 1 * DIN_dim -> batch_size * DIN_dim
        h_e = tf.reshape(tf.matmul(h_e, trans_M), [-1, self.DIN_dim])
        pos_t_e = tf.reshape(tf.matmul(pos_t_e, trans_M), [-1, self.DIN_dim])
        neg_t_e = tf.reshape(tf.matmul(neg_t_e, trans_M), [-1, self.DIN_dim])
    ...
    def _build_loss_phase_I(self):
        ...
        self.DIN_loss = tf.constant(0.0, tf.float32, [1])
        self.reg_loss = self.regs[0] * regularizer
        self.loss = self.base_loss + self.DIN_loss + self.reg_loss

        # Optimization process.RMSPropOptimizer
        self.opt = tf.train.AdamOptimizer(learning_rate=self.lr).minimize(self.loss)

    def _build_loss_phase_II(self):
        def _get_DIN_score(h_e, r_e, t_e):
            DIN_score = tf.reduce_sum(tf.square((h_e + r_e - t_e)), 1, keepdims=True)
            return DIN_score

        pos_DIN_score = _get_DIN_score(self.h_e, self.r_e, self.pos_t_e)
        neg_DIN_score = _get_DIN_score(self.h_e, self.r_e, self.neg_t_e)

        maxi = tf.log(tf.nn.sigmoid(neg_DIN_score - pos_DIN_score))
        DIN_loss = tf.negative(tf.reduce_mean(maxi))
        DIN_reg_loss = tf.nn.l2_loss(self.h_e) + tf.nn.l2_loss(self.r_e) + \
                      tf.nn.l2_loss(self.pos_t_e) + tf.nn.l2_loss(self.neg_t_e)
        DIN_reg_loss = DIN_reg_loss / self.batch_size


        self.DIN_loss2 = DIN_loss
        self.reg_loss2 = self.regs[1] * DIN_reg_loss
        self.loss2 = self.DIN_loss2 + self.reg_loss2
        self.mutu_loss = self.cal_mutual_loss(self.get_glo_emb(self.A_in),self.ua_embeddings)
    ...
    def _create_bi_interaction_embed(self):
        ...
        ego_embeddings = tf.concat([self.weights['drug_embed'], self.weights['entity_embed']], axis=0)
        ...
        ua_embeddings, ea_embeddings = tf.split(all_embeddings, [self.n_drugs, self.n_entities], 0)
    ...
    def _create_graphsage_embed(self):
        ...
        pre_embeddings = tf.concat([self.weights['drug_embed'], self.weights['entity_embed']], axis=0)
    ...
    def _split_A_hat(self, X):
        ...
        fold_len = (self.n_drugs + self.n_entities) // self.n_fold

        for i_fold in range(self.n_fold):
            start = i_fold * fold_len
            if i_fold == self.n_fold - 1:
                end = self.n_drugs + self.n_entities
            else:
    ...
    def _generate_transE_score(self, h, t, r):
        ...
        # batch_size * 1 * DIN_dim -> batch_size * DIN_dim
        h_e = tf.reshape(tf.matmul(h_e, trans_M), [-1, self.DIN_dim])
        t_e = tf.reshape(tf.matmul(t_e, trans_M), [-1, self.DIN_dim])
        ...
        DIN_score = tf.reduce_sum(tf.multiply(t_e, tf.tanh(h_e + r_e)), 1)
    ...
    def train(self, sess, feed_dict):
        return sess.run([self.opt, self.loss, self.base_loss, self.DIN_loss, self.reg_loss], feed_dict)

    def train_A(self, sess, feed_dict):
        return sess.run([self.opt2, self.loss2, self.DIN_loss2, self.reg_loss2, self.mutu_loss], feed_dict)
    ...
    def update_attentive_A(self, sess):
        ...
        for i_fold in range(self.n_fold):
            ...
            A_DIN_score = sess.run(self.A_DIN_score, feed_dict=feed_dict)
            DIN_score += list(A_DIN_score)

        DIN_score = np.array(DIN_score)

        new_A = sess.run(self.A_out, feed_dict={self.A_values: DIN_score})
        new_A_values = new_A.values
        new_A_indices = new_A.indices

        rows = new_A_indices[:, 0]
        cols = new_A_indices[:, 1]
        self.A_in = sp.coo_matrix((new_A_values, (rows, cols)), shape=(self.n_drugs + self.n_entities,
                                                                       self.n_drugs + self.n_entities))
